...
/*
* 目前wpa3存在的问题（nm-applet 的agent也存在下面的问题）：
                                            ...
                                            NMVariantMapMap &secretMap)
{
    ...
    if (PSK_MGMT == secretType || SAE_MGMT == secretType) {
        askSecretForPassword(wirelessSecurityMap, PSK_KEY, settingName, secretMap);
    } else if (WEP_MGMT == secretType || WEP128_MGMT == secretType) {
        askSecretForWep(wirelessSecurityMap, settingName, secretMap);
    } else if (LEAP_MGMT == secretType) {
        askSecretForLeap(wirelessSecurityMap, settingName, secretMap);
    } else {
                                            ...
                                            NMVariantMapMap &secretMap)
{
    ...
    if(pwdDialog.exec()==QDialog::Accepted){
        ...
        /*
        *  如果用户关闭窗口，或者是取消等操作，清空secretmap，后面通过sendError返回结果
        */
        secretMap.clear();
    }
                                                      ...
                                                      NMVariantMapMap &secretMap)
{
    ...
    if (TLS_EAP == secretType) {
        askSecretForTls(enterpriceSecurityMap, settingName, secretMap);
    } else {
        askSecretWithIdentityAndPassword(enterpriceSecurityMap,
                                    IDENTITY_KEY, PASSWORD_KEY, settingName, secretMap);
    }
...
对于下面这种密码类型nm-applet的agnet没有保存，不处理
802-1x.password-raw:                    --
