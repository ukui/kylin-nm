...
void SecurityPage::setFastInfo(KyEapMethodFastInfo &info)
{
    ...
    if (info.m_pacProvisioning == KyFastProvisioningDisabled) {
        ...
        m_pacCheckBox->setChecked(info.m_allowAutoPacFlag);
        m_pacProvisionComboBox->setCurrentIndex(ANON);
        m_pacProvisionComboBox->setEnabled(info.m_allowAutoPacFlag);
...
void SecurityPage::setAutoConnectVisible(bool visible)
{
    m_checkLabel->setVisible(visible);
    m_rememberCheckBox->setVisible(visible);
}

void SecurityPage::setOnlyPasswordEditable(bool enabled)
{
    const bool editable = !enabled;
    secuTypeCombox->setEnabled(editable);
    pwdEdit->setEnabled(editable);
    eapTypeCombox->setEnabled(editable);

    identityEdit->setEnabled(editable);
    domainEdit->setEnabled(editable);
    caCertPathCombox->setEnabled(editable);
    caNeedBox->setEnabled(editable);
    clientCertPathCombox->setEnabled(editable);
    clientPrivateKeyCombox->setEnabled(editable);
    clientPrivateKeyPwdEdit->setEnabled(editable);
    pwdOptionCombox->setEnabled(editable);

    eapMethodCombox->setEnabled(editable);
    userNameEdit->setEnabled(editable);
    userPwdEdit->setEnabled(editable);
    userPwdFlagBox->setEnabled(editable);

    m_pacCheckBox->setEnabled(editable);
    m_pacProvisionComboBox->setEnabled(editable);
    m_pacFilePathComboBox->setEnabled(editable);
    m_rememberCheckBox->setEnabled(editable);

    if (!enabled) {
        return;
    }

    const int secType = secuTypeCombox->currentData().toInt();
    if (secType == KYLIN_NM::WPA_AND_WPA2_PERSONAL || secType == KYLIN_NM::WPA3_PERSONAL) {
        pwdEdit->setEnabled(true);
        pwdEdit->setFocus();
        return;
    }

    if (secType == KYLIN_NM::WPA_AND_WPA2_ENTERPRISE) {
        const int eapType = eapTypeCombox->currentData().toInt();
        if (eapType == TLS) {
            clientPrivateKeyPwdEdit->setEnabled(true);
            clientPrivateKeyPwdEdit->setFocus();
        } else {
            userPwdEdit->setEnabled(true);
