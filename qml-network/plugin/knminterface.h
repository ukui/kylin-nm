...
public slots:
    ...
    /** 与 WlanPage::m_expandedItem 对齐：展开期间暂停列表重排，仅原地更新属性；折叠后全量对齐 Keeper */
    void setWirelessListReorderPaused(bool paused);
    ...
    void showPasswordError(const QString &requestId, const QString &deviceName, const QString &ssid, const QString &password);
    void submitWirelessPassword(const QString &uuid, const QString &ssid, const QString &password);
    void cancelWirelessPassword(const QString &uuid, const QString &ssid);
...
signals:
    ...
    void deviceSwitched(QString devName, int type);

    void passwordErrorOccurred(const QString &ssid, const QString &password);
private:
    ...
    bool m_wirelessListReorderPaused = false;
    bool m_wirelessResumingList = false;
};
