...
运行
麒麟网络工具的进程为kylin-nm，默认开机自启，依赖NetworkManager
...
getStoredApInfo
参数：无
返回值：QStringList（名称/密码/设备名称/状态/UUID/频带）
功能：获取已保存的移动热点信息

getApInfoBySsid(QString devName, QString ssid)
参数：QString devName 设备名称 QString ssid 无线网络SSID
返回值：无
功能：通过SSID获取已保存的移动热点信息
...
调试
kylin-nm目前采用ukui-log4qt模块的日志功能。日志默认保存在~/.log/kylin-nm.log中
