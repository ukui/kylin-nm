...
void DbusAdaptor::sendPasswordError(const QString &requestId, const QString &deviceName, const QString &ssid, const QString &password, const QVariantMap &enterpriseInfo)
{
    qDebug() << Q_FUNC_INFO << "emit sigPasswordError"<<deviceName<<ssid<<password;

    if (!enterpriseInfo.isEmpty()) {
        // 企业WiFi重连，传递完整信息
        m_mainWindow->handleEnterpriseWifiReconnection(requestId, deviceName, ssid, password, enterpriseInfo);
    }else{
        Q_EMIT DbusAdaptor::sigPasswordError(requestId, deviceName, ssid, password);
    }
}

void DbusAdaptor::submitWirelessPassword(const QString &requestId, const QString &deviceName, const QString &ssid, const QString &password)
{
    if (m_mainWindow && m_mainWindow->secretAgent()) {
        qDebug() << "submitWirelessPassword" << deviceName << ssid << password;
        QVariantMap secretMap;
        secretMap[PASSWORD_KEY] = password;
        m_mainWindow->secretAgent()->onSubmitPassword(requestId, ssid, secretMap);
    }
}

void DbusAdaptor::cancelWirelessPassword(const QString &requestId, const QString &deviceName, const QString &ssid)
{
    if (m_mainWindow && m_mainWindow->secretAgent()) {
        qDebug() << "cancelWirelessPassword" << deviceName << ssid<<m_mainWindow<<m_mainWindow->secretAgent();
        m_mainWindow->secretAgent()->onCancelPassword(requestId, ssid);
    }
}
...
//刷新托盘图标
void DbusAdaptor::refreshTrayIcon()
{
    qDebug()<< Q_FUNC_INFO << __LINE__ << "refreshTrayIcon";
    ...
    if (switchBtnState) {
        ...
        if ((valueMap.size() <= 2) || (valueMap.size() >= 3 && valueMap.at(2).toString().isEmpty())) { //如果默认设备未获取到连接，则遍历其他网卡设备
            for (const auto &data : devDatalist) {
                ...
                if (0 == type)
                    connectList = getWiredList(data);
                else if (1 == type)
                    connectList = getWirelessList(data);
