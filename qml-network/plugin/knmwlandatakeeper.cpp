...
// 测试模式标志
static bool g_wlanDataKeeperTestMode = false;
//unit test 没有 wifi 模块测试
// LCOV_EXCL_START
void KnmWlanDataKeeper::setTestMode(bool enable)
{
    g_wlanDataKeeperTestMode = enable;
}

bool KnmWlanDataKeeper::isTestMode()
{
    return g_wlanDataKeeperTestMode;
}

KnmWlanDataKeeper::KnmWlanDataKeeper(QObject *parent) : KnmDataKeeper(parent)
{
    // 测试模式下跳过D-Bus初始化
    if (!g_wlanDataKeeperTestMode) {
    ...
    QDBusConnection::sessionBus().connect("com.kylin.network",
                                         ...
                                         "sigPasswordError",
                                         this,
                                         SLOT(onPasswordError(QString, QString, QString, QString)));
...
void KnmWlanDataKeeper::onPasswordError(QString requestId, QString deviceName, QString ssid, QString password)
{
     // 通过D-Bus信号转发到KInterface
     qDebug() << "Password error for device:" << deviceName << "ssid:" << ssid<<"password:"<<password;

     KInterface::getInstance()->showPasswordError(requestId, deviceName, ssid, password);
}
// LCOV_EXCL_STOP
