...
private:
    ...
    void askSecretWithIdentityAndPassword(QVariantMap securityMap, QString userKey,
                               QString passwordKey, const QString &settingName, NMVariantMapMap &secretMap);
...
public Q_SLOTS:
     ...
     * 注意事项：
     * 1）所有的函数对keyring的操作都是同步的，需要其单独的agent线程，否则容易阻塞
     * 2）目前代码只是对能够测试到的情况做了代码实现，有可能会有遗漏的地方，后续出现新情况，在完善。
    */

    /*
     * 该函数从秘钥环上面获取密码，并将获取到的密码组成networkamanger需要的mapmap类型返回，失败后通过sendError返回错误，
     * 函数的返回值将变得无效
    */
